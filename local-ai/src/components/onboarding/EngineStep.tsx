...
export function EngineStep({ onNext, onBack }: EngineStepProps) {
  ...
  return (
    <StepShell
      ...
      description={
        IS_MAC_MODEL_PROVIDER
          ...
          : 'ModernClaw talks to the local engine on your machine to run the model layer.'
      }
      backLabel="Back"
    ...
    >
      {isChecking ? (
        <StatusCard
          tone="neutral"
          title={IS_MAC_MODEL_PROVIDER ? 'Checking for Direct Engine...' : 'Checking for Engine...'}
        ...
        <StatusCard
          ...
          description={
            engineStatus?.version
              ? `Version detected: ${engineStatus.version}. The next step is ${
        ...
        <StatusCard
          tone="warning"
          title={IS_MAC_MODEL_PROVIDER ? 'Direct Engine not detected' : 'Engine not detected'}
          description={
            IS_MAC_MODEL_PROVIDER
              ? 'Start the local llama.cpp engine on port 8080 with a Gemma 4 model, then check again.'
              : 'Install and start the local engine, then check again. ModernClaw depends on it for model execution.'
          }
        >
          <div className="mt-5 rounded-2xl bg-background/60 p-4 text-left text-sm leading-7 text-muted-foreground">
            <ol className="list-decimal space-y-1 pl-5">
              {IS_MAC_MODEL_PROVIDER ? (
                ...
                <>
                  <li>
                    Install the local engine (llama.cpp) from{' '}
                    <a
                  ...
                  <li>Start the engine on this machine.</li>
          ...
          <div className="mt-6">
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <Button variant="outline" onClick={() => void startEngine()} disabled={isStartingEngine}>
